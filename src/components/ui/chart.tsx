...
function ChartTooltipContent({
	...
	return (
		<div className={cn(
			"grid items-start gap-1.5 min-w-32 px-2.5 py-1.5 text-xs text-popover-foreground ring-1 ring-foreground/5 dark:ring-foreground/10 rounded-xl shadow-lg",
					...
					.map((item, index) => {
						...
						return (
							<div
								key={index}
								className={cn(
									"flex flex-wrap items-stretch gap-1.5 w-full",
									...
									<>
										{itemConfig?.icon ? (
											...
											!hideIndicator && (
												<div
													className={cn(
														"shrink-0 bg-(--color-bg) border-(--color-border) rounded-xs",
														{
										...
										<div className={cn(
											"flex-1 flex gap-4 justify-between leading-none",
					...
					return (
						...
						>
							{itemConfig?.icon && !hideIcon ? (
								...
								<div
									className="shrink-0 size-2.5 rounded-xs"
